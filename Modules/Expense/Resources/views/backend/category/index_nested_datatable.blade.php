...
@push('after-styles')
    <link rel="stylesheet" href="{{ mix('modules/expense/style.css') }}">
@endpush
...
@section('content')
        ...
        <div class="card-body">
            <x-backend.section-header>
              <div class="d-flex flex-wrap gap-3">
                <x-backend.quick-action url='{{route("backend.expenses-categories.bulk_action")}}'>
                  <div class="">
    ...
    <div data-render="app">
          <expense-category-form-offcanvas :is-sub-category="true" create-title="{{ __('messages.new') }} {{ __($module_title) }}"
            edit-title="{{ __('messages.edit') }} {{ __($module_title) }}" :category-id="{{ $category->id ?? 'null' }}">
        </expense-category-form-offcanvas>
    </div>
@endsection
...
@push('after-scripts')
    <script src="{{ mix('modules/expense/script.js') }}"></script>
    ...
    const columns = [
            ...
            { data: 'name', name: 'name',  title: "{{ __('category.lbl_name') }}" ,width: '30%',  orderable: true },
